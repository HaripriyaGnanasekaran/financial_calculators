...
class HistoricalReturnStd(CalculationNode):
    ...
    def plot(self):
        ...
        t_params = t.fit(dr)
        lognorm_params = lognorm.fit(dr)
        print(t_params)

        plt.hist(self.data[market_df][cv.daily_return], bins=1000, density=True, alpha=0.9, label="Daily return")
        #plt.plot(x, pdf, 'r', label="Normal")
        plt.plot(x, t.pdf(x, *t_params), label="Student's t-distribution")
        plt.plot(x, lognorm.pdf(x, *lognorm_params), label="Lognormal distribution")
