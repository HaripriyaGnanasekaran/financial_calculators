...
from calculation.variables.levels import monte_carlo_df, metrics_dict
from calculation.variables import computed_variables as cv
from calculation.parameters import parameters as p
from calculation.variables import domain_values as dv
...
class PopulateMonteCarloDF(CalculationNode):
    ...
    def execute(self):
        ...
        mean = self.data[metrics_dict][cv.historical_return_tmean] if p.fit == dv.student_t else self.data[metrics_dict][cv.historical_return_mean]
        std = self.data[metrics_dict][cv.historical_return_tstd] if p.fit == dv.student_t else self.data[metrics_dict][cv.historical_return_std]
        ...
        for scenario in tqdm(range(p.scenarios), desc="Simulating scenarios"):
            ...
            random_t_returns = self._get_random_returns(degree_of_freedom, mean, std, n_days)
            cumulative_returns = pd.Series(random_t_returns + 1).cumprod()
            scenario_returns[:, scenario] = cumulative_returns * self.data[metrics_dict][cv.current_stock_price]

        scenario_columns = [f"price_{scenario}" for scenario in range(p.scenarios)]
        self.data[monte_carlo_df] = pd.DataFrame(scenario_returns, columns=scenario_columns)
